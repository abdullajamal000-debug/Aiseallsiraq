...
def process_image(image_bytes: bytes) -> bytes:
    """تفريغ خلفية صورة المنتج وتركيبها على خلفية استوديو احترافية مجاناً"""
    # 1. فتح صورة المنتج الأصلي
    input_img = Image.open(io.BytesIO(image_bytes)).convert("RGBA")
    
    # 2. إزالة الخلفية بالذكاء الاصطناعي
    no_bg_img = remove(input_img)
    
    # 3. إنشاء خلفية استوديو ناعمة تليق بالمكياج والعطور (لون رمادي داكن فخم/أو أوف وايت)
    # يمكنك تغيير اللون (RGB): (245, 245, 247) لون فاتح احترافي
    bg_color = (245, 245, 247, 255) 
    background = Image.new("RGBA", no_bg_img.size, bg_color)
    
    # 4. دمج المنتج المفرغ فوق الخلفية
    final_img = Image.alpha_composite(background, no_bg_img)
    
    # 5. تحويلها لملف جاهز للإرسال
